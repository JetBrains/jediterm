# BossTerm Architecture Analysis

## Project Overview
BossTerm is a pure Java terminal emulator with a clean separation between core terminal emulation logic and Swing-based UI implementation. The codebase is organized into three main modules:
- **core**: Terminal emulation engine (framework-agnostic)
- **ui**: Swing/AWT-based UI components
- **BossTerm**: Demo application and utilities
...
2. `ai.rever.bossterm.terminal.emulator`
   - `BossEmulator.java` - Main ANSI escape sequence parser and processor
...
3. `ai.rever.bossterm.terminal.model`
...
4. `ai.rever.bossterm.terminal.emulator.mouse`
   - Mouse mode/format handling

5. `ai.rever.bossterm.terminal.emulator.charset`
   - Character set support (G0, G1, etc.)
   - DEC box drawing characters

6. `ai.rever.bossterm.core`
...
**Key Packages**:
1. `ai.rever.bossterm.terminal.ui` (27 UI classes)
   ...
   - Other UI components:
     - `BossTermSearchComponent.java` - Search interface
     - `BossTermDefaultSearchComponent.java` - Default search impl
...
2. `ai.rever.bossterm.terminal.ui.settings`
...
```
Input Events → TerminalStarter → BossEmulator → Terminal Interface
                                                      ↓
...
|-------|-----------------|
| `BossEmulator` | Parse and execute ANSI escape sequences |
...
| `BossTermWidget` | JPanel container for panel + scrollbar + search |
...
- `javax.swing.JPanel` - BossTermWidget base
