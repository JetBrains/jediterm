...
public class ScrollBuffer implements StyledTextConsumer {
  ...
  public synchronized String getLines() {
    ...
    final StyledTextConsumer consumer = new StyledTextConsumer() {
      public void consume(int x, int y, TextStyle style, BufferCharacters characters) {
        if (x == 0) sb.append('\n');
  ...
  public synchronized void addToBuffer(TextStyle style, BufferCharacters characters, boolean isNewLine) {
  ...
  static class Section {
    ...
    private int pumpRunsImpl(final int firstLine,
      ...
      for (int i = 0; i < runStarts.length; i++) {
        ...
        consumer.consume(x, y, runStyles[i], new BufferCharacters(buf, runStart, runEnd - runStart));
        x += runEnd - runStart;
        if (last) break;
      }
      return y;
